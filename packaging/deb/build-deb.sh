...
PUBLISH_DIR="${REPOSITORY_ROOT}/artifacts/publish"
...
#!/usr/bin/env bash
exec /usr/bin/dotnet /opt/${PACKAGE_NAME}/LinuxBuildLab.Sample.dll "\$@"
WRAPPER
...
Maintainer: Linux Build Lab
Description: One-shot .NET sample package for Linux Build Lab
 A small framework-dependent .NET application used to verify
 Debian package creation, installation, execution, and removal.
CONTROL
...
#!/usr/bin/env bash
set -e
if systemctl is-enabled --quiet linux-build-lab-sample.service; then
  systemctl disable linux-build-lab-sample.service || true
fi
exit 0
PRERM
